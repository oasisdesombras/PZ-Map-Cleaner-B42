...
La herramienta definitiva para realizar limpiezas (wipes) selectivas de zonas del mapa en la **Build 42 Unstable**. Diseñada específicamente para manejar la nueva estructura de carpetas que rompió las herramientas de la B41.
...
* **B42 Ready:** Única herramienta que limpia correctamente las cuatro nuevas subcarpetas de datos: `/map`, `/zpop`, `/chunkdata` y **`/apop/`**.
* **Gestión de Animales (Nuevo):** Permite resetear opcionalmente los datos de animales (`apop`) para optimizar el rendimiento del servidor y eliminar el lag por sobrepoblación.
* **Interfaz Visual:** Selección de áreas intuitiva sobre el mapa para evitar errores de coordenadas.
* **Conserva el progreso:** El wipe **no afecta** al archivo `players.db`. Los jugadores mantienen sus estadísticas, habilidades y objetos equipados.

---

## ⚠️ Notas de la Versión (Beta)
* **Claims:** Actualmente el proceso no diferencia zonas protegidas por jugadores. Se recomienda avisar a la comunidad antes de realizar el wipe.
* **Animales:** Al seleccionar el borrado de datos de animales, estos reaparecerán según la configuración original de la zona.
* **Seguridad:** ¡Haz siempre un **backup** de tu carpeta de guardado antes de operar! 
  - `Ruta: Saves/Multiplayer/NombreDelServidor`
